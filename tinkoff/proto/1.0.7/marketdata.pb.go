...
const (
	...
	SubscriptionStatus_SUBSCRIPTION_STATUS_SUBSCRIPTION_ACTION_IS_INVALID SubscriptionStatus = 3 //Некорректный статус подписки, список возможных значений: [SubscriptionAction](https://tinkoff.github.io/proto/marketdata#subscriptionaction).
	...
	SubscriptionStatus_SUBSCRIPTION_STATUS_INTERVAL_IS_INVALID            SubscriptionStatus = 5 //Некорректный интервал свечей, список возможных значений: [SubscriptionInterval](https://tinkoff.github.io/proto/marketdata#subscriptioninterval).
	SubscriptionStatus_SUBSCRIPTION_STATUS_LIMIT_IS_EXCEEDED              SubscriptionStatus = 6 //Превышен лимит подписок в рамках стрима, подробнее: [Лимитная политика](https://tinkoff.github.io/proto/limits/).
...
type SubscribeCandlesResponse struct {
	...
	TrackingId           string                `protobuf:"bytes,1,opt,name=tracking_id,json=trackingId,proto3" json:"tracking_id,omitempty"`                               //Уникальный идентификатор запроса, подробнее: [tracking_id](https://tinkoff.github.io/proto/grpc#tracking-id).
